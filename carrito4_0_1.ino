...
void ejecutarGiro(int grados) {
  ...
  // Fórmula de tiempo reducida y más predecible
  // CONSTANTE A CALIBRAR: Cambia este '3' si gira de más o de menos.
  int tiempoGiro = (abs(grados) * 4) + 20; 
  registrarEvento(
    "AUTO: Giro " + String(grados) +
...
void left(){ 
  analogWrite(IN1, 0); 
  analogWrite(IN2, velocidadGiro);  // Usando velocidadGiro
  analogWrite(IN3, velocidadGiro);  // Usando velocidadGiro
  analogWrite(IN4, 0); 
}
void right(){ 
  analogWrite(IN1, velocidadGiro);  // Usando velocidadGiro
  analogWrite(IN2, 0); 
  analogWrite(IN3, 0); 
  analogWrite(IN4, velocidadGiro);  // Usando velocidadGiro
}
void stopCar(){ 
  analogWrite(IN1, 0); 
  analogWrite(IN2, 0); 
